...
- **🤖 一句话让 AI 填充模版（本模版与其他模版的最大区别）**：下载本仓库的
  `skills/` 文件夹并安装到 [Claude Code](https://claude.com/claude-code)（安装命令见下文）后，
  只需对 Claude 说一句话——例如"用科大模版把这份组会大纲做成 12 页 PPT"——它就会自动
  挑选合适的布局页、复制并把你的内容填进去；其中的公式会自动经 LaTeX 渲染为透明 PNG、
  按原始比例插入（浅底深字、深底白字），目录与导航条的跳转链接也会自动重新接好
...
- 版式设计基于同学 **zxc** 制作的北京科技大学（USTB）模版改造，特此致谢
